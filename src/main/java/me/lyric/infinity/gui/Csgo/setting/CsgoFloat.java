...
public class CsgoFloat extends CsgoSetting
{
    public int extension;
    public float indicator;
    FloatSetting setting;
    
    public CsgoFloat(final FloatSetting setting) {
        super(setting);
    ...
    public void drawScreen(final int mouseX, final int mouseY) {
        ...
        if (this.indicator < sliderWidth) {
            this.indicator = AnimationUtils.increaseNumber(this.indicator, sliderWidth, (sliderWidth - this.indicator) / CsgoGui.getAnimationSpeedAccordingly(100));
        }
        else if (this.indicator > sliderWidth) {
            this.indicator = AnimationUtils.decreaseNumber(this.indicator, sliderWidth, (this.indicator - sliderWidth) / CsgoGui.getAnimationSpeedAccordingly(100));
        }
        ...
        String string = this.setting.getName() + ": " + this.roundNumber(this.setting.getValue(), 1);
    ...
    public float sliderWidthValue() {
        return (this.setting.getValue() - this.setting.getMinimum()) / (this.setting.getMaximum() - this.setting.getMinimum());
    }
    ...
    public void setSliderValue(final int mouseX) {
        ...
        final float min = this.setting.getMinimum();
        final float max = this.setting.getMaximum();
        if (diff == 0.0f) {
            this.setting.setValue(this.setting.getMinimum());
        }
        else {
