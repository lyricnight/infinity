...
public class WindowUtil implements IGlobals {
    ...
    public WindowUtil(int slot, ItemStack inSlot, ItemStack inDrag) {
        this(slot, inSlot, inDrag, slot);
    }

    public WindowUtil(int slot, ItemStack inSlot, ItemStack inDrag, int target) {
        this(slot, inSlot, inDrag, target,
                p -> p.windowClick(0, slot, 0, ClickType.PICKUP, mc.player));
    }

    public WindowUtil(int slot, ItemStack inSlot, ItemStack inDrag, int target, Consumer<PlayerControllerMP> action) {
        this(slot, inSlot, -2, inDrag, target, action);
    }

    public WindowUtil(int slot, ItemStack inSlot, int dragSlot, ItemStack inDrag, int target, Consumer<PlayerControllerMP> action) {
        this.slot = slot;
        this.inSlot = inSlot;
        this.inDrag = inDrag;
        this.target = target;
    ...
    public void runClick(PlayerControllerMP controller) {
        ...
        if (slot != -1 && slot != -999 && fast && (!InventoryUtil.equals(stack, mc.player.inventory.getItemStack()) || !InventoryUtil.equals(drag, InventoryUtil.get(slot)))) {
