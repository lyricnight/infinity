...
public class CustomFontRenderer extends CustomFont
{
    ...
    {
        drawString(text, x - getStringWidth(text) / 2.0f + 1.0, y + 1.0, color, true);
        return drawString(text, x - getStringWidth(text) / 2.0f, y, color);
    }
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow)
    {
        ...
        if (shadow)
        {
            if (Managers.MODULES.getModuleByClass(Fonts.class).shadow.getValue())
            {
                x -= 0.4;
                y -= 0.4;
            }
        ...
        for (int i = 0; i < text.length(); i++)
        {
            ...
            if (character == TextColor.SECTIONSIGN && i + 1 < text.length())
            {
                ...
                else if (colorIndex == 22)
                {
                    ...
                    GlStateManager.color((colorcode >> 16 & 0xFF) / 255.0f / (shadow ? 4 : 1), (colorcode >> 8 & 0xFF)  / 255.0f / (shadow ? 4 : 1), (colorcode & 0xFF)       / 255.0f / (shadow ? 4 : 1), (colorcode >> 24 & 0xFF) / 255.0f);
                ...
                {
                    Infinity.LOGGER.error("Incorrect color index called! - 23");
                }
                else
                {
                    Infinity.LOGGER.error("Hit CustomFontRenderer else!");
                }
            ...
            else if (character < currentData.length)
            {
                ...
                if (strike)
                {
                    drawLine(x, y + currentData[character].height / 2.0, x + currentData[character].width - 8.0, y + currentData[character].height / 2.0);
                }
    ...
    private void setupBoldItalicIDs()
    {
        ...
        texBoth = setupTexture(this.font.deriveFont(Font.BOLD | Font.ITALIC), this.antiAlias, this.fractionalMetrics, this.boldItalicChars);
    }
    ...
    public List<String> wrapWords(String text, double width)
    {
        ...
        if (getStringWidth(text) > width || text.contains("\n"))
        {
            ...
            for (String word : words)
            {
                ...
                if (lineBreak.length > 1)
                {
                    if (getStringWidth(current + lineBreak[0] + " ") >= width)
                    {
                        ...
                        current = new StringBuilder().append(TextColor.SECTIONSIGN).append(lastColorCode).append(lineBreak[0]);
                    }
                    else
                    {
                    ...
                    {
                        result.add(current.toString());
                        current = new StringBuilder().append(TextColor.SECTIONSIGN).append(lastColorCode).append(lineBreak[i]);
                ...
                {
                    result.add(current.toString());
                    current = new StringBuilder().append(TextColor.SECTIONSIGN).append(lastColorCode).append(word).append(" ");
                }
            }
            ...
            if (current.length() > 0)
            {
                ...
                {
                    result.add(TextColor.SECTIONSIGN + "" + lastColorCode + current + " ");
                }
                else
                {
    ...
    public List<String> formatString(String string, double width)
    {
        ...
        for (int i = 0; i < string.length(); i++)
        {
            ...
            {
                result.add(current.toString());
                current = new StringBuilder().append(TextColor.SECTIONSIGN).append(lastColorCode).append(c);
            }
        }
